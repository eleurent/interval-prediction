...
\]
for some $a_{0},a_{s},\phi_{s}\in\R$ with $s=\overline{s_{1},s_{2}}$ and $||d||\leq W$.\footnote{We present this finite spectrum setting for the sake of simplicity, but other assumptions could be made on $d$ instead, such as $2\pi$-periodicity. Their relevance depend on the particular application considered.} Then for any $x(0)\in\R^{n}$ in \eqref{eq:LTI_syst} and any $\epsilon>0$ there exists $\tau>0$ such that:
\[
