...
\title{\LARGE \bf
Improved interval prediction for continuous-time LPV systems}
...
\thispagestyle{empty}
\begin{abstract}
This work studies the behaviour of linear parameter-varying systems subject to uncertain inputs. It is only assumed that the unknown system parameters and inputs belong to some known admissible set, and our objective is to predict a tight interval hull bounding the system trajectories. Having observed the instability of traditional interval predictor techniques, we propose a new one whose stability is guaranteed by a Lyapunov function analysis and verification of linear matrix inequalities. In the case when an information is available about the frequency spectrum of the input signals, the predictor can also be enhanced with an improved asymptotic accuracy. We demonstrate the efficacy of our solution by applying it to a problem of robust motion planning for a self-driving car.
\end{abstract}


\section{Introduction}

There are plenty of emerging applicative domains nowadays, where the decision algorithms have to operate in the conditions of a severe uncertainty. To mention a few: biological systems and control of bioreactors (where it is hard to establish detailed models and it is impossible to isolate and evaluate most of the external to the plant perturbations) and path planning for self-driving cars (where despite of a large parametric variation of the models, the presence of humans in the loop introduces a lot of imprecision due to their spontaneous reactions). Therefore, the decision procedures need more information, then the estimation, identification and prediction algorithms come to the attention. In most of these applications, even the nominal simplified models are nonlinear, and in order to solve the problem of estimation and control in nonlinear and uncertain systems, a popular approach is based on the Linear Parameter-Varying (LPV) representation of their dynamics \cite{Shamma2012,Marcos_Balas04,Shamma_Cloutier93,Tan97}, since it allows to reduce the problem to the linear context at the price of augmented parametric variation.
...
\begin{figure}
    ...
    \centering
      \includegraphics[width=0.49\textwidth]{driving_observer_3lanes.png}
      \label{sub:hw-a}
    }
    \newline
     \subfloat[The proposed predictor \eqref{eq:IP_main} remains stable]{
    \centering
      \includegraphics[width=0.49\textwidth]{driving_predictor_3lanes.png}
      \label{sub:hw-b}
}
\newline
    \subfloat[Prediction during a lane change maneuver]{
    \centering
      \includegraphics[width=0.49\textwidth]{lane_change_predictor_3lanes.png}
      \label{sub:hw-c}
}
\newline
