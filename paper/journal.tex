...
\thispagestyle{empty}
\begin{abstract}
This work studies the behaviour of uncertain linear systems subject to uncertain inputs. It is only assumed that the unknown system parameters and inputs belong to some known admissible set, and our objective is to predict a tight interval hull bounding the system trajectories. Having observed the instability of traditional interval observer techniques, we propose a novel interval predictor whose stability is guaranteed by a Lyapunov function in the form of linear matrix inequalities. In the case when an information is available about the frequency spectrum of the input signals, the predictor can be also enhanced with an improved asymptotic accuracy. We demonstrate the efficacy of our solution to a problem of robust motion planning for a self-driving car.
\end{abstract}
